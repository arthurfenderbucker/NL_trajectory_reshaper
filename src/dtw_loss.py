...
def batch_soft_dtw(X, Y, gamma, warp, metric="L2"):
    ...
    def _batch_soft_dtw_kernel(delta_matrix, raw_delta_matrix):
        ...
        def grad_v1(dy):
            ...
            tmp_grad = tf.linalg.matmul(tmp_grad, tf.ones(shape=[tf.shape(raw_delta_matrix_v1)[0], tf.shape(raw_delta_matrix_v1)[2], tf.shape(raw_delta_matrix_v1)[1]], dtype=tf.float32))

            return (tmp_grad,tmp_grad)
...
if __name__ == '__main__':
    ...
    #  sequence1
    a = tf.Variable(np.random.rand(2, n, d))

    #  sequence2(or target sequence)
    b = np.random.rand(2, m, d)
