...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, input_vocab_size=2, num_layers=4, d_model=512, num_heads=8, dff=2048, maximum_position_encoding=10000, dropout=0.0, **kwargs):
        ...
        self.encoder_layers = [EncoderLayer(
            d_model=d_model, num_heads=num_heads, dff=dff, dropout=dropout) for _ in range(num_layers)]
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, target_vocab_size=2, num_layers=4, d_model=512, num_heads=8, dff=2048, maximum_position_encoding=10000, dropout=0.0, **kwargs):
    ...
    def call(self, inputs, mask=None, training=None):
        ...
        feature_vec = self.feature_embedding(inputs[2])*0.0
...
def get_model(features_n=777, num_layers_enc=2, num_layers_dec=2,num_dense=3,dense_n=256, d_model=128, dff=512, num_heads=8, dropout_rate=0.1, wp_d=2,bs=32, concat_emb=False, optimizer="adam"):
    ...
    x = tf.keras.layers.Dense(target_vocab_size, activation="linear")(x)
